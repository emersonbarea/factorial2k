...
function update_SO_install_packages() {
    ...
    sudo apt upgrade -y
    sudo apt install git vim htop ethtool sysfsutils python-pip python3-pip ifupdown mc strace tcpdump netcat nmap python-scapy whois -y
}
...
function install_app_nps() {
    ...
    sudo apt install cmake default-jdk git ant libgl1-mesa-dev freeglut3-dev libgstreamer1.0-dev libgstreamermm-1.0-dev libwebkitgtk-dev -y
